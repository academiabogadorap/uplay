...
def _has_index(table: str, idx_name: str) -> bool:
    """Chequea existencia de índice en SQLite vía PRAGMA; en otros, intenta metadata."""
    bind = op.get_bind()
    ...
    # En otros motores, intentamos un SELECT a catálogo cuando esté disponible,
    # pero por simplicidad lo dejamos a manejo de excepciones en _safe_drop_index.
    return True  # se intentará y si no existe, se ignora en _safe_drop_index


def _safe_drop_index(idx_name: str, table: str):
    """Borra índice si existe. Evita flush diferido dentro de batch."""
    bind = op.get_bind()
    if bind.dialect.name == "sqlite":
        # SQLite soporta IF EXISTS; hacerlo fuera de batch evita el flush diferido
        op.execute(f'DROP INDEX IF EXISTS "{idx_name}"')
    else:
        # En otros motores no todos soportan IF EXISTS en DROP INDEX portable.
        # Probamos a eliminar; si no existe, ignoramos.
        try:
...
def upgrade():
    ...
    # --- Limpieza defensiva por si quedaron tablas temporales de intentos previos (SQLite)
    if is_sqlite:
        for t in (
            "_alembic_tmp_desafios",
            "_alembic_tmp_torneos",
            "_alembic_tmp_torneos_inscripciones",
            "_alembic_tmp_partidos",
            "_alembic_tmp_jugadores",
            "_alembic_tmp_partido_resultado_propuesto",
            "_alembic_tmp_partido_abierto_jugadores",
        ):
    ...
    # JUGADORES
    # ---------------------------
    # ⚠️ Drop del índice problemático: hacerlo **fuera** del batch y condicional
    if _has_index("jugadores", "ux_jugadores_email"):
        _safe_drop_index("ux_jugadores_email", "jugadores")
    ...
    with op.batch_alter_table("jugadores", schema=None) as batch_op:
        ...
        # No hacemos drop_index aquí para evitar flush diferido en SQLite.
    ...
    # ---------------------------
    # Drop índice potencialmente inexistente: hacerlo fuera del batch y condicional
    if _has_index("partido_resultado_propueso", "ix_prp_creado_en"):  # nombre de tabla correcto abajo
        _safe_drop_index("ix_prp_creado_en", "partido_resultado_propuesto")

    with op.batch_alter_table("partido_resultado_propuesto", schema=None) as batch_op:
        # Nada que dropear aquí para evitar flush diferido.
        pass

    # ---------------------------
    # PARTIDOS
    # ---------------------------
    # Si había índices preexistentes, dropearlos condicionalmente **antes** del batch
    for idx in (
        "ix_partidos_creador",
        "ix_partidos_estado",
        ...
        if _has_index("partidos", idx):
            _safe_drop_index(idx, "partidos")
    ...
    if not is_sqlite:
        ...
        with op.batch_alter_table("partidos", schema=None) as batch_op:
            ...
            batch_op.create_foreign_key(
                "fk_partidos_rechazo_ultimo_por_id_jugadores", "jugadores", ["rechazo_ultimo_por_id"], ["id"]
            )
            batch_op.create_foreign_key("fk_partidos_creador_id_jugadores", "jugadores", ["creador_id"], ["id"])
            batch_op.create_foreign_key(
                "fk_partidos_resultado_propuesto_por_id_jugadores",
                "jugadores",
...
def downgrade():
    ...
    # PARTIDO_RESULTADO_PROPUESTO
    # recrear índice si aplica
    try:
        op.create_index(op.f("ix_prp_creado_en"), "partido_resultado_propuesto", ["creado_en"], unique=False)
    except Exception:
        pass
