...
try:
    ...
    from models import *
    from utils import *
    print("✅ Módulos importados correctamente desde config, models y utils.")
except Exception as e:
...
if __name__ == "__main__":
    try:
        print("🚀 Ejecutando verificación de Flask...")
        import subprocess
        result = subprocess.run(["flask", "--app", "app", "routes"], capture_output=True, text=True)
        if result.returncode == 0:
            print("✅ Flask se ejecutó correctamente.")
            print(result.stdout)
        else:
            print("⚠️ Flask devolvió un error, revisá el log abajo:")
            print(result.stderr)
